...
namespace turbo::vMemory {
	...
	PTable *get_next_lvl(PTable *curr_lvl, size_t entry){
		...
		if (curr_lvl->entries[entry].getFlag(present)){
			ret = (PTable*)(((uint64_t)curr_lvl->entries[entry].getAddress()) << 12);
		}
		else{
			ret = (PTable*)pMemory::alloc();
			curr_lvl->entries[entry].setAddress(((uint64_t)ret) >> 12);
	...
	Pagemap *newPagemap(){
		...
		if (kernel_pagemap == nullptr){
			pagemap->TOPLVL = (PTable*)read_cr(3);
			return pagemap;
		}

		pagemap->TOPLVL = (PTable*)pMemory::alloc();
	...
	void switchPagemap(Pagemap *pmap){
		write_cr(3, (uint64_t)pmap->TOPLVL);
	}
